...
BEGIN
    IF NOT EXISTS ( -- Table exists
        ...
        WHERE
            table_schema = 'cds2db_in'
            AND table_name = 'temp_calculated_items') THEN
    ...
    GRANT INSERT, DELETE, UPDATE, SELECT ON TABLE cds2db_in.temp_calculated_items TO cds2db_user;
    -- Additional authorizations for testing
END IF;
    -- Table exists
    CREATE OR REPLACE VIEW cds2db_out.v_temp_calculated_items AS (
        SELECT
            *
        FROM
            cds2db_in.temp_calculated_items);
        GRANT SELECT ON TABLE cds2db_out.v_temp_calculated_items TO cds2db_user;
