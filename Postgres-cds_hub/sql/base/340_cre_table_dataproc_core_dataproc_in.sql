    ...
    IF EXISTS ( -- Table exists
        ...
        FROM (
            ...
            WHERE
                ...
                AND table_name = 'input_data_files') a,
            (
                SELECT
                    1 s
                FROM
        ...
        IF NOT EXISTS ( -- column not exists
            ...
            WHERE
                table_schema = 'db2dataprocessor_in'
                AND table_name = 'input_data_files'
...
BEGIN
    IF EXISTS ( -- Table exists
        ...
        FROM (
            ...
            WHERE
                table_schema = 'db2dataprocessor_in'
                AND table_name = 'input_data_files_processed_content') a,
            (
                SELECT
                    1 s
                FROM
        ...
        IF NOT EXISTS ( -- column not exists
            ...
            WHERE
                table_schema = 'db2dataprocessor_in'
                AND table_name = 'input_data_files_processed_content'
                AND column_name = 'input_datetime') THEN
        NULL;
END IF;
    -- column
...
GRANT INSERT, DELETE, UPDATE, SELECT ON TABLE db2dataprocessor_in.input_data_files TO db2dataprocessor_user;

-- Additional authorizations for testing
GRANT INSERT, DELETE, UPDATE, SELECT ON TABLE db2dataprocessor_in.input_data_files TO db_user;

-- Additional authorizations for testing
GRANT SELECT ON TABLE db2dataprocessor_in.input_data_files TO db_log_user;
...
GRANT INSERT, DELETE, UPDATE, SELECT ON TABLE db2dataprocessor_in.input_data_files_processed_content TO db2dataprocessor_user;

-- Additional authorizations for testing
GRANT INSERT, DELETE, UPDATE, SELECT ON TABLE db2dataprocessor_in.input_data_files_processed_content TO db_user;

-- Additional authorizations for testing
GRANT SELECT ON TABLE db2dataprocessor_in.input_data_files_processed_content TO db_log_user;
        ...
        IF EXISTS ( -- target column
            ...
            IF EXISTS ( -- INDEX available
                ...
                FROM
                    pg_indexes
                WHERE
                    substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_input_dt', 1, 63)
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'input_data_files') THEN
                ...
                IF EXISTS ( -- INDEX nicht auf akuellen Stand
                    ...
                    WHERE
                        ...
                        AND schemaname = 'db2dataprocessor_in'
                        AND tablename = 'input_data_files'
                        AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_input_dt', 1, 63)
                        AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_input_data_files_input_dt ON db2dataprocessor_in.input_data_files USING brin (input_datetime)') THEN
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_input_pnr', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'input_data_files') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'input_data_files'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_input_pnr', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_input_data_files_input_pnr ON db2dataprocessor_in.input_data_files USING brin (input_processing_nr)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_last_dt', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'input_data_files') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'input_data_files'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_last_dt', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_input_data_files_last_dt ON db2dataprocessor_in.input_data_files USING brin (last_check_datetime)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_last_pnr', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'input_data_files') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'input_data_files'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_last_pnr', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_input_data_files_last_pnr ON db2dataprocessor_in.input_data_files USING brin (last_processing_nr)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_hash', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'input_data_files') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'input_data_files'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_hash', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_input_data_files_input_dt ON db2dataprocessor_in.input_data_files USING btree (hash_index_col)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_processed_content_input_dt', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'input_data_files_processed_content') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                SELECT
                    1
                ...
                WHERE
                    schemaname NOT IN ('pg_catalog', 'information_schema')
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'input_data_files_processed_content'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_processed_content_input_dt', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_input_data_files_processed_content_input_dt ON db2dataprocessor_in.input_data_files_processed_content USING brin (input_datetime)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_processed_content_input_pnr', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'input_data_files_processed_content') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'input_data_files_processed_content'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_processed_content_input_pnr', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_input_data_files_processed_content_input_pnr ON db2dataprocessor_in.input_data_files_processed_content USING brin (input_processing_nr)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_processed_content_last_dt', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'input_data_files_processed_content') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'input_data_files_processed_content'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_processed_content_last_dt', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_input_data_files_processed_content_last_dt ON db2dataprocessor_in.input_data_files_processed_content USING brin (last_check_datetime)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_processed_content_last_pnr', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'input_data_files_processed_content') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'input_data_files_processed_content'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_processed_content_last_pnr', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_input_data_files_processed_content_last_pnr ON db2dataprocessor_in.input_data_files_processed_content USING brin (last_processing_nr)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_processed_content_hash', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'input_data_files_processed_content') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'input_data_files_processed_content'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_input_data_files_processed_content_hash', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_input_data_files_processed_content_input_dt ON db2dataprocessor_in.input_data_files_processed_content USING btree (hash_index_col)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
