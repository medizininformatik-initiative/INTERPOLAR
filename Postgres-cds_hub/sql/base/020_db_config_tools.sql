...
SELECT
    ...
    (
        ...
        FROM
            cron.job_run_details s
        WHERE
            command = m.command
            AND s.status = 'succeeded') last_succeeded_run,
    (
        SELECT
            to_char(min(s.end_time), 'YYYY-MM-DD HH24:MI:SS')
        FROM
            cron.job_run_details s
        WHERE
            command = m.command
            AND s.status = 'succeeded') first_succeeded_run,
    (
        SELECT
            to_char(max(s.end_time), 'YYYY-MM-DD HH24:MI:SS')
        FROM
        ...
        WHERE
            command = m.command
            AND s.status != 'succeeded') last_faild_run
FROM
    cron.job_run_details m
GROUP BY
    command
ORDER BY
    3 DESC;
...
BEGIN
    IF EXISTS (
        ...
        FROM
            cron.job
        WHERE
            command = 'DELETE FROM cron.job_run_details WHERE status=''succeeded'' AND end_time < now() - interval ''2 days'''
        LIMIT 1) THEN
SELECT
    res
FROM
    public.pg_background_result (public.pg_background_launch ('SELECT cron.schedule(''0 0 * * *'', DELETE FROM cron.job_run_details WHERE status=''succeeded'' AND end_time < now() - interval ''2 days'');')) AS t (res TEXT)
INTO
    erg;
END IF;
END
...
BEGIN
    ...
    IF EXISTS ( -- Zielspalte existiert
        ...
        WHERE
            ...
            AND table_name = 'db_parameter'
            AND column_name = 'parameter_name') THEN
        IF EXISTS ( -- INDEX vorhanden
            ...
            FROM
                pg_indexes
            WHERE
                indexname = 'idx_db_config_db_parameter_name') THEN
            ...
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db_config'
                    AND tablename = 'db_parameter'
                    AND indexname = 'idx_db_config_db_parameter_name'
                    AND indexdef != 'CREATE INDEX idx_db_config_db_parameter_name ON db_config.db_parameter USING btree (parameter_name)') THEN
    ...
    IF EXISTS ( -- Zielspalte existiert
        ...
        IF EXISTS ( -- INDEX vorhanden
            ...
            FROM
                pg_indexes
            WHERE
                indexname = 'idx_db_config_db_db_process_control_name') THEN
            -- aktuellen Stand überprüfen
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                SELECT
                    1
                ...
                WHERE
                    ...
                    AND schemaname = 'db_config'
                    AND tablename = 'db_parameter'
                    AND indexname = 'idx_db_config_db_db_process_control_name'
                    AND indexdef != 'CREATE INDEX idx_db_config_db_db_process_control_name ON db_config.db_process_control USING btree (pc_name)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
...
BEGIN
    IF NOT EXISTS (
        SELECT
            1
        FROM
            db_config.db_process_control
        WHERE
            pc_name = 'semaphor_cron_job_data_transfer') THEN
...
END IF;
    ...
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_process_control
        WHERE
            pc_name = 'timepoint_1_cron_job_data_transfer') THEN
    INSERT INTO db_config.db_process_control (pc_name, pc_value, pc_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_process_control
        WHERE
            pc_name = 'timepoint_2_cron_job_data_transfer') THEN
    INSERT INTO db_config.db_process_control (pc_name, pc_value, pc_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_process_control
        WHERE
            pc_name = 'timepoint_3_cron_job_data_transfer') THEN
    INSERT INTO db_config.db_process_control (pc_name, pc_value, pc_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_process_control
        WHERE
            pc_name = 'waitpoint_cron_job_data_transfer') THEN
    INSERT INTO db_config.db_process_control (pc_name, pc_value, pc_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_process_control
        WHERE
            pc_name = 'current_executed_function') THEN
    INSERT INTO db_config.db_process_control (pc_name, pc_value, pc_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_process_control
        WHERE
            pc_name = 'current_total_number_of_records_in_the_function') THEN
    INSERT INTO db_config.db_process_control (pc_name, pc_value, pc_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_process_control
        WHERE
            pc_name = 'currently_processed_number_of_data_records_in_the_function') THEN
    INSERT INTO db_config.db_process_control (pc_name, pc_value, pc_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_process_control
        WHERE
            pc_name = 'semaphor_last_block_modul') THEN
    INSERT INTO db_config.db_process_control (pc_name, pc_value, pc_description)
...
BEGIN
    ...
    IF EXISTS ( -- Zielspalte existiert
        ...
        WHERE
            table_schema = 'db'
            AND table_name = 'data_import_hist'
        ...
        IF EXISTS ( -- INDEX vorhanden
            ...
            FROM
                pg_indexes
            WHERE
                indexname = 'idx_db_data_import_hist_last_processing_nr') THEN
            -- aktuellen Stand überprüfen
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                SELECT
                    1
                ...
                WHERE
                    schemaname NOT IN ('pg_catalog', 'information_schema')
                    AND schemaname = 'db'
                    AND tablename = 'data_import_hist'
                    AND indexname = 'idx_db_data_import_hist_last_processing_nr'
                    AND indexdef != 'CREATE INDEX idx_db_data_import_hist_last_processing_nr ON db.data_import_hist USING btree (last_processing_nr)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- Zielspalte existiert
        ...
        IF EXISTS ( -- INDEX vorhanden
            ...
            FROM
                pg_indexes
            WHERE
                indexname = 'idx_db_data_import_hist_schema_name') THEN
            -- aktuellen Stand überprüfen
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                SELECT
                    1
                ...
                WHERE
                    ...
                    AND schemaname = 'db'
                    AND tablename = 'data_import_hist'
                    AND indexname = 'idx_db_data_import_hist_schema_name'
                    AND indexdef != 'CREATE INDEX idx_db_data_import_hist_schema_name ON db.data_import_hist USING btree (schema_name)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- Zielspalte existiert
        ...
        IF EXISTS ( -- INDEX vorhanden
            ...
            FROM
                pg_indexes
            WHERE
                indexname = 'idx_db_data_import_hist_table_name') THEN
            -- aktuellen Stand überprüfen
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                SELECT
                    1
                ...
                WHERE
                    ...
                    AND schemaname = 'db'
                    AND tablename = 'data_import_hist'
                    AND indexname = 'idx_db_data_import_hist_table_name'
                    AND indexdef != 'CREATE INDEX idx_db_data_import_hist_table_name ON db.data_import_hist USING btree (table_name)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
...
END IF;
    ...
    IF EXISTS ( -- Zielspalte existiert
        ...
        WHERE
            table_schema = 'db'
            AND table_name = 'data_import_hist'
            AND column_name = 'function_name') THEN
        IF EXISTS ( -- INDEX vorhanden
            SELECT
                1
            FROM
                pg_indexes
            WHERE
                indexname = 'idx_db_data_import_hist_function_name') THEN
            -- aktuellen Stand überprüfen
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                SELECT
                    1
                ...
                WHERE
                    ...
                    AND schemaname = 'db'
                    AND tablename = 'data_import_hist'
                    AND indexname = 'idx_db_data_import_hist_function_name'
                    AND indexdef != 'CREATE INDEX idx_db_data_import_hist_function_name ON db.data_import_hist USING btree (function_name)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
...
END IF;
    ...
    IF EXISTS ( -- Zielspalte existiert
        ...
        WHERE
            table_schema = 'db'
            AND table_name = 'data_import_hist'
            AND column_name = 'variable_name') THEN
        IF EXISTS ( -- INDEX vorhanden
            SELECT
                1
            FROM
                pg_indexes
            WHERE
                indexname = 'idx_db_data_import_hist_variable_name') THEN
            -- aktuellen Stand überprüfen
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                SELECT
                    1
                ...
                WHERE
                    ...
                    AND schemaname = 'db'
                    AND tablename = 'data_import_hist'
                    AND indexname = 'idx_db_data_import_hist_variable_name'
                    AND indexdef != 'CREATE INDEX idx_db_data_import_hist_variable_name ON db.data_import_hist USING btree (variable_name)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
...
SELECT
    a.*,
    b.dataset_count_new_ds,
    b.new_ds_per_sec
FROM (
    SELECT
        to_char(import_hist_cre_at, 'YYYY-MM-DD') day_sum,
        function_name,
        CASE WHEN function_name = 'copy_raw_cds_in_to_db_log' THEN
            'CDS2DB_IN (RAW) -> DB_LOG'
        WHEN function_name = 'copy_type_cds_in_to_db_log' THEN
            'CDS2DB_IN (Typed) -> DB_LOG'
        WHEN function_name = 'copy_fe_dp_in_to_db_log' THEN
            'DP (Typed) -> DB_LOG-Frontend'
        WHEN function_name = 'copy_fe_fe_in_to_db_log' THEN
            'DB_LOG-Frontend -> DB_LOG'
        WHEN function_name = 'take_over_last_check_date' THEN
            'DB_LOG (internal) - Synchronization of the last imported data sets'
        ELSE
            'NichtDefinierteFunktion'
        END dataflow,
        sum(dataset_count) dataset_count_all_ds,
        round(sum(copy_time_in_sec)) copy_time_in_sec,
        round(sum(dataset_count) / sum(copy_time_in_sec)) all_ds_per_sec
    FROM
        db.data_import_hist a
    WHERE
        variable_name = 'data_count_pro_all'
    GROUP BY
        function_name,
        to_char(import_hist_cre_at, 'YYYY-MM-DD')) a
    LEFT JOIN (
        SELECT
            to_char(import_hist_cre_at, 'YYYY-MM-DD') day_sum,
            function_name,
            sum(dataset_count) dataset_count_new_ds,
            round(sum(copy_time_in_sec)) copy_time_in_sec,
            round(sum(dataset_count) / sum(copy_time_in_sec)) new_ds_per_sec
        FROM
            db.data_import_hist a
        WHERE
            variable_name = 'data_count_pro_new'
        GROUP BY
            function_name,
            to_char(import_hist_cre_at, 'YYYY-MM-DD')) b ON (a.day_sum = b.day_sum
...
------------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION db.error_log (err_schema varchar DEFAULT current_schema, -- Schema in which the error occurred
...
BEGIN
    ...
    SELECT
        res
    FROM
        public.pg_background_result (public.pg_background_launch ('INSERT INTO db_config.db_error_log (err_schema,err_objekt,err_user,err_msg,err_line,err_variables,last_processing_nr) VALUES (''' || err_schema || ''', ''' || err_objekt || ''', ''' || err_user || ''', ''' || err_msg || ''', ''' || err_line || ''', ''' || err_variables || ''',' || COALESCE(last_processing_nr, 'NULL') || ')')) AS t (res TEXT)
INTO
    erg;
EXCEPTION
    WHEN OTHERS THEN
...
CREATE OR REPLACE FUNCTION db.to_char_immutable (input_data numeric)
    ...
    AS $$
    SELECT
        to_char(input_data, 'FM999999999999990.999999');
$$
LANGUAGE SQL
IMMUTABLE;
...
CREATE OR REPLACE FUNCTION db.to_char_immutable (input_data timestamp)
    ...
    AS $$
    SELECT
        to_char(input_data, 'YYYY-MM-DD HH24:MI:SS.US');
$$
LANGUAGE SQL
IMMUTABLE;
...
CREATE OR REPLACE FUNCTION db.to_char_immutable (input_data time)
    ...
    AS $$
    SELECT
        to_char(input_data, 'HH24:MI:SS.US');
$$
LANGUAGE SQL
IMMUTABLE;
...
CREATE OR REPLACE FUNCTION db.to_char_immutable (input_data boolean)
    ...
    AS $$
    SELECT
        CASE WHEN input_data THEN
            'true'
        WHEN NOT input_data THEN
            'false'
        ELSE
            'null'
        END;
