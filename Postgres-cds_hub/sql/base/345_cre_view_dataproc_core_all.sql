            ...
            IF EXISTS ( -- do migration
                ...
                IF EXISTS ( -- VIEW exists
                    ...
                    WHERE
                        ...
                        AND table_name = 'v_input_data_files') THEN
                    DROP VIEW db2dataprocessor_out.v_input_data_files;
            -- first drop the view
        END IF;
    -- DROP VIEW
    ----------------------------
    CREATE OR REPLACE VIEW db2dataprocessor_out.v_input_data_files AS (
        SELECT
            *
        FROM
            db_log.input_data_files);
    ...
    BEGIN
        IF EXISTS ( -- do migration
            ...
            IF EXISTS ( -- VIEW exists
                ...
                WHERE
                    table_schema = 'db2dataprocessor_out'
                    AND table_name = 'v_input_data_files_processed_content') THEN
                DROP VIEW db2dataprocessor_out.v_input_data_files_processed_content;
        -- first drop the view
    END IF;
        -- DROP VIEW
        ----------------------------
        CREATE OR REPLACE VIEW db2dataprocessor_out.v_input_data_files_processed_content AS (
            SELECT
                *
            FROM
                db_log.input_data_files_processed_content);
