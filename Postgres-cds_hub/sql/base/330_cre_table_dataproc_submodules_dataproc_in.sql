...
END IF;
    ...
    IF NOT EXISTS ( -- column not exists
        ...
        WHERE
            ...
            AND table_name = 'dp_mrp_calculations'
            AND column_name = 'input_datetime') THEN
        ALTER TABLE db2dataprocessor_in.dp_mrp_calculations
...
GRANT INSERT, DELETE, UPDATE, SELECT ON TABLE db2dataprocessor_in.dp_mrp_calculations TO db2dataprocessor_user;

-- Additional authorizations for testing
GRANT INSERT, DELETE, UPDATE, SELECT ON TABLE db2dataprocessor_in.dp_mrp_calculations TO db_user;

-- Additional authorizations for testing
GRANT SELECT ON TABLE db2dataprocessor_in.dp_mrp_calculations TO db_log_user;
        ...
        IF EXISTS ( -- target column
            ...
            IF EXISTS ( -- INDEX available
                ...
                FROM
                    pg_indexes
                WHERE
                    substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_dp_mrp_calculations_input_dt', 1, 63)
                ...
                IF EXISTS ( -- INDEX nicht auf akuellen Stand
                    ...
                    WHERE
                        ...
                        AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_dp_mrp_calculations_input_dt', 1, 63)
                        AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_dp_mrp_calculations_input_dt ON db2dataprocessor_in.dp_mrp_calculations USING brin (input_datetime)') THEN
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_dp_mrp_calculations_input_pnr', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'dp_mrp_calculations') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'dp_mrp_calculations'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_dp_mrp_calculations_input_pnr', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_dp_mrp_calculations_input_pnr ON db2dataprocessor_in.dp_mrp_calculations USING brin (input_processing_nr)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_dp_mrp_calculations_last_dt', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'dp_mrp_calculations') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'dp_mrp_calculations'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_dp_mrp_calculations_last_dt', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_dp_mrp_calculations_last_dt ON db2dataprocessor_in.dp_mrp_calculations USING brin (last_check_datetime)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_dp_mrp_calculations_last_pnr', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'dp_mrp_calculations') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'dp_mrp_calculations'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_dp_mrp_calculations_last_pnr', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_dp_mrp_calculations_last_pnr ON db2dataprocessor_in.dp_mrp_calculations USING brin (last_processing_nr)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_dp_mrp_calculations_hash', 1, 63)
                AND schemaname = 'db2dataprocessor_in'
                AND tablename = 'dp_mrp_calculations') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db2dataprocessor_in'
                    AND tablename = 'dp_mrp_calculations'
                    AND substr(indexname, 1, 63) = substr('idx_db2dataprocessor_in_dp_mrp_calculations_hash', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_dp_mrp_calculations_input_dt ON db2dataprocessor_in.dp_mrp_calculations USING btree (hash_index_col)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
        ...
        IF EXISTS ( -- INDEX available
            ...
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND indexdef != 'CREATE INDEX idx_dp_mrp_calculations_ret_redcap_repeat_instance ON db2dataprocessor_in.dp_mrp_calculations USING btree (ret_redcap_repeat_instance)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
