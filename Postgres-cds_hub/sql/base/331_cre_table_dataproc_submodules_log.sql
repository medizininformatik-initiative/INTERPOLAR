...
-- Create time: 2026-02-26 16:07:09
...
-- Data-leading columns -------------------------------------------------------------------------
        ...
        IF NOT EXISTS ( -- column not exists (atc1_medreq_fhir_id)
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db_log' AND table_name = 'dp_mrp_calculations' AND column_name = 'atc1_medreq_fhir_id'
        ) THEN
            ALTER TABLE db_log.dp_mrp_calculations ADD atc1_medreq_fhir_id varchar;   -- optional – FHIR ID of ATC1 (varchar)
        ...
        IF NOT EXISTS ( -- column not exists (mrp_proxy_fhir_id)
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db_log' AND table_name = 'dp_mrp_calculations' AND column_name = 'mrp_proxy_fhir_id'
        ) THEN
            ALTER TABLE db_log.dp_mrp_calculations ADD mrp_proxy_fhir_id varchar;   -- optional – FHIR ID of the proxy (varchar)
