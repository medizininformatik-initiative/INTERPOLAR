...
BEGIN
    IF EXISTS ( -- Table exists
        ...
        FROM (
            ...
            WHERE
                table_schema = 'db_log'
                AND table_name = 'dp_mrp_calculations') a,
            (
                SELECT
                    1 s
                FROM
        ...
        IF NOT EXISTS ( -- column not exists
            ...
            WHERE
                table_schema = 'db_log'
                AND table_name = 'dp_mrp_calculations'
...
GRANT INSERT, DELETE, UPDATE, SELECT ON TABLE db_log.dp_mrp_calculations TO db_log_user;

-- Additional authorizations for testing
GRANT INSERT, DELETE, UPDATE, SELECT ON TABLE db_log.dp_mrp_calculations TO db_user;
...
BEGIN
    ...
    COMMENT ON COLUMN db_log.dp_mrp_calculations.atc1_medreq_fhir_id IS 'optional – FHIR ID of ATC1 (varchar)';
    ...
    COMMENT ON COLUMN db_log.dp_mrp_calculations.mrp_proxy_fhir_id IS 'optional – FHIR ID of the proxy (varchar)';
    ...
    IF EXISTS ( -- do migration
        ...
        IF EXISTS ( -- target column
            ...
            WHERE
                table_schema = 'db_log'
                AND table_name = 'dp_mrp_calculations'
                AND column_name = 'dp_mrp_calculations_id') THEN
            IF EXISTS ( -- INDEX available
                ...
                FROM
                    pg_indexes
                WHERE
                    substr(indexname, 1, 63) = substr('idx_dp_mrp_calculations_id', 1, 63)
                    AND schemaname = 'db_log'
                    AND tablename = 'dp_mrp_calculations') THEN
                ...
                IF EXISTS ( -- INDEX nicht auf akuellen Stand
                    ...
                    WHERE
                        ...
                        AND schemaname = 'db_log'
                        AND tablename = 'dp_mrp_calculations'
                        AND substr(indexname, 1, 63) = substr('idx_dp_mrp_calculations_id', 1, 63)
                        AND indexdef != 'CREATE INDEX idx_dp_mrp_calculations_id ON db_log.dp_mrp_calculations USING btree (dp_mrp_calculations_id DESC)') THEN
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db_log_dp_mrp_calculations_input_dt', 1, 63)
                AND schemaname = 'db_log'
                AND tablename = 'dp_mrp_calculations') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db_log'
                    AND tablename = 'dp_mrp_calculations'
                    AND substr(indexname, 1, 63) = substr('idx_db_log_dp_mrp_calculations_input_dt', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db_log_dp_mrp_calculations_input_dt ON db_log.dp_mrp_calculations USING brin (input_datetime)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db_log_dp_mrp_calculations_input_pnr', 1, 63)
                AND schemaname = 'db_log'
                AND tablename = 'dp_mrp_calculations') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db_log'
                    AND tablename = 'dp_mrp_calculations'
                    AND substr(indexname, 1, 63) = substr('idx_db_log_dp_mrp_calculations_input_pnr', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db_log_dp_mrp_calculations_input_pnr ON db_log.dp_mrp_calculations USING brin (input_processing_nr)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db_log_dp_mrp_calculations_last_dt', 1, 63)
                AND schemaname = 'db_log'
                AND tablename = 'dp_mrp_calculations') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db_log'
                    AND tablename = 'dp_mrp_calculations'
                    AND substr(indexname, 1, 63) = substr('idx_db_log_dp_mrp_calculations_last_dt', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db_log_dp_mrp_calculations_last_dt ON db_log.dp_mrp_calculations USING brin (last_check_datetime)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db_log_dp_mrp_calculations_last_pnr', 1, 63)
                AND schemaname = 'db_log'
                AND tablename = 'dp_mrp_calculations') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db_log'
                    AND tablename = 'dp_mrp_calculations'
                    AND substr(indexname, 1, 63) = substr('idx_db_log_dp_mrp_calculations_last_pnr', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db_log_dp_mrp_calculations_last_pnr ON db_log.dp_mrp_calculations USING brin (last_processing_nr)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_db_log_dp_mrp_calculations_hash', 1, 63)
                AND schemaname = 'db_log'
                AND tablename = 'dp_mrp_calculations') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db_log'
                    AND tablename = 'dp_mrp_calculations'
                    AND substr(indexname, 1, 63) = substr('idx_db_log_dp_mrp_calculations_hash', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_db_log_dp_mrp_calculations_input_dt ON db_log.dp_mrp_calculations USING btree (hash_index_col)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
    ...
    IF EXISTS ( -- target column
        ...
        IF EXISTS ( -- INDEX available
            ...
            FROM
                pg_indexes
            WHERE
                substr(indexname, 1, 63) = substr('idx_dp_mrp_calculations_ret_redcap_repeat_instance', 1, 63)
                AND schemaname = 'db_log'
                AND tablename = 'dp_mrp_calculations') THEN
            -- check current status
            IF EXISTS ( -- INDEX nicht auf akuellen Stand
                ...
                WHERE
                    ...
                    AND schemaname = 'db_log'
                    AND tablename = 'dp_mrp_calculations'
                    AND substr(indexname, 1, 63) = substr('idx_dp_mrp_calculations_ret_redcap_repeat_instance', 1, 63)
                    AND indexdef != 'CREATE INDEX idx_dp_mrp_calculations_ret_redcap_repeat_instance ON db_log.dp_mrp_calculations USING btree (ret_redcap_repeat_instance)') THEN
                -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
