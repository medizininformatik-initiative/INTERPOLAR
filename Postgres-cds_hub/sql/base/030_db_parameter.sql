...
DECLARE
    ...
    release_version varchar := '2.2.0';
    -- GitHup release version
    release_version_nr int := 220;
    -- version number - first 3 digits as integer for comparison
    release_version_date varchar := '2026-09-18';
    ...
    data_import_hist_every_dataset varchar := 'no';
    -- Documentation of each individual data record (db_log.data_import_hist) in all the transfer functions [yes|no] - large resource requirements only for debugging
    max_process_time_set_ready varchar := '120';
    -- Maximum time that the semaphore may remain in use before it is released again in minutes [5-120]
    copy_fhir_metadata_from_raw_to_typed varchar := 'N';
    -- Optionale Einstellung (Yes/No) ob die FHIR-Metadaten beim Kopiervorgang auch von RAW in die TYPED Tabellen übernommen werden soll.
    current_migration_flag varchar := '0';
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE
            parameter_name = 'database_initialization_time') THEN
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'last_valid_release_version') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'last_valid_release_version_date') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'last_valid_release_version_log') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'release_version') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'release_version_last_migration_start') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
...
ELSE
    UPDATE
        db_config.db_parameter
    SET
        parameter_value = release_version
    WHERE
        parameter_name = 'release_version_last_migration_start';
END IF;
    IF NOT EXISTS (
        SELECT
            1
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'release_version_nr') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
        VALUES ('release_version_nr', 0, 'version number (first 3 digits)');
    -- If initialized, a new database can be assumed
END IF;
    IF NOT EXISTS (
        SELECT
            1
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'release_version_nr_last_migration_start') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
...
ELSE
    UPDATE
        db_config.db_parameter
    SET
        parameter_value = release_version_nr
    WHERE
        parameter_name = 'release_version_nr_last_migration_start';
END IF;
    IF NOT EXISTS (
        SELECT
            1
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'current_migration_flag') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'last_migration_date') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
...
ELSE
    UPDATE
        db_config.db_parameter
    SET
        parameter_value = to_char(CURRENT_TIMESTAMP, 'YYYY-MM-DD HH24:MI:SS')
    WHERE
        parameter_name = 'last_migration_date';
END IF;
    IF NOT EXISTS (
        SELECT
            1
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'release_version_date') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
...
END IF;
    IF NOT EXISTS (
        ...
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'release_version_date_last_migration_start') THEN
    INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
...
ELSE
    UPDATE
        db_config.db_parameter
    SET
        parameter_value = release_version_date
    WHERE
        parameter_name = 'release_version_date_last_migration_start';
END IF;
    ...
    IF EXISTS (
        ...
        FROM
            db_config.db_parameter
        WHERE
            parameter_name = 'release_version_nr'
            AND CAST(parameter_value AS integer) < release_version_nr) THEN
    UPDATE
        db_config.db_parameter
    SET
        parameter_value = '1'
    WHERE
        parameter_name = 'current_migration_flag';
        -- do - skirpt is newer then in database
        SELECT
            '1'
        INTO
            current_migration_flag;
        UPDATE
            db_config.db_parameter
        SET
            parameter_value = release_version_nr
    ...
    ELSE
        ...
        WHERE
            parameter_name = 'current_migration_flag';
        -- dont - skript is older or same then in database
        SELECT
            '0'
        INTO
            current_migration_flag;
END IF;
        IF current_migration_flag = '1' THEN
            ...
            IF NOT EXISTS (
                ...
                FROM
                    db_config.db_parameter
                WHERE
                    parameter_name = 'pause_after_process_execution') THEN
            INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
        ...
        ELSE
            UPDATE
                db_config.db_parameter
            SET
                parameter_value = pause_after_process_execution
            WHERE
                parameter_name = 'pause_after_process_execution';
        END IF;
        IF NOT EXISTS (
            SELECT
                1
            FROM
                db_config.db_parameter
            WHERE
                parameter_name = 'data_import_hist_every_dataset') THEN
        INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
    ...
    ELSE
        UPDATE
            db_config.db_parameter
        SET
            parameter_value = data_import_hist_every_dataset
        WHERE
            parameter_name = 'data_import_hist_every_dataset';
END IF;
        IF NOT EXISTS (
            SELECT
                1
            FROM
                db_config.db_parameter
            WHERE
                parameter_name = 'max_process_time_set_ready') THEN
        INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
    ...
    ELSE
        UPDATE
            db_config.db_parameter
        SET
            parameter_value = max_process_time_set_ready
        WHERE
            parameter_name = 'max_process_time_set_ready';
END IF;
        IF NOT EXISTS (
            SELECT
                1
            FROM
                db_config.db_parameter
            WHERE
                parameter_name = 'copy_fhir_metadata_from_raw_to_typed') THEN
        INSERT INTO db_config.db_parameter (parameter_name, parameter_value, parameter_description)
    ...
    ELSE
        ...
        SET
            parameter_value = max_process_time_set_ready
        WHERE
            parameter_name = 'copy_fhir_metadata_from_raw_to_typed';
END IF;
END IF;
        -- current_migration_flag
END
