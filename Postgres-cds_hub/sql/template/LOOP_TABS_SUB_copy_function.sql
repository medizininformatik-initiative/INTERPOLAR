...
<%IF RIGHTS_DEFINITION:TAGS "\bTYPED\b" "                            -- Store earlier raw_id in raw if dataset comes from other raw_dataset (raw_already_processed)
                            err_section:='<%TABLE_NAME%>-30';    err_schema:='<%OWNER_SCHEMA%>';    err_table:='<%TABLE_NAME%>';
                            SELECT count(1) INTO data_count FROM <%OWNER_SCHEMA%>.<%TABLE_NAME%> target_record
                            WHERE target_record.hash_index_col = current_record.hash_index_col AND target_record.<%SIMPLE_TABLE_NAME%>_raw_id = current_record.<%SIMPLE_TABLE_NAME%>_raw_id;
                            IF data_count = 0 THEN -- Retrieve the last raw_id that generated the same hash and is not the current raw_id
                                err_section:='<%TABLE_NAME%>-33';    err_schema:='<%SCHEMA_2%>';    err_table:='<%TABLE_NAME_2%>';
                                SELECT count(1) INTO data_count FROM <%OWNER_SCHEMA%>.<%TABLE_NAME%> target_record
                                WHERE target_record.hash_index_col = current_record.hash_index_col AND target_record.<%SIMPLE_TABLE_NAME%>_raw_id < current_record.<%SIMPLE_TABLE_NAME%>_raw_id;
                                ...
                                ELSE -- find last raw_id to hash and set as processed flag
                                    SELECT max(<%SIMPLE_TABLE_NAME%>_raw_id) INTO data_count FROM <%OWNER_SCHEMA%>.<%TABLE_NAME%> target_record
                                    WHERE target_record.hash_index_col = current_record.hash_index_col AND target_record.<%SIMPLE_TABLE_NAME%>_raw_id < current_record.<%SIMPLE_TABLE_NAME%>_raw_id;
                                END IF;
                                err_section:='<%TABLE_NAME%>-35';    err_schema:='<%SCHEMA_2%>';    err_table:='<%TABLE_NAME_2%>';
                                UPDATE <%OWNER_SCHEMA%>.<%TABLE_NAME%>_raw SET raw_already_processed = data_count WHERE <%SIMPLE_TABLE_NAME%>_raw_id = current_record.<%SIMPLE_TABLE_NAME%>_raw_id AND (raw_already_processed < data_count OR raw_already_processed IS NULL);
                            END IF;"%>
