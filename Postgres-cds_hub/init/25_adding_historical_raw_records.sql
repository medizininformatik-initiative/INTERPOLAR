...
--	SELECT patient_raw_id,
--  !!! Aufzählung der FHIR Spalten fehlt noch !!!
--        'reimported from database'
--        FROM cds2db_out.v_patient_raw_last_version q    -- View mit letzten FHIR Daten 
--        WHERE q.pat_patient_ref IN (SELECT pat_id FROM cds2db_in.patient_raw) -- Für alle aktuellen Patienten
--        AND q.hash_index_col NOT IN (SELECT hash_index_col FROM cds2db_in.patient_raw) -- wenn nocht nicht vorhanden durch laden vom FHIR Server
...
--	SELECT medication_raw_id,
--  !!! Aufzählung der FHIR Spalten fehlt noch !!!
--        'reimported from database'
--        FROM cds2db_out.v_medication_raw_last_version q    -- View mit letzten FHIR Daten 
--        WHERE q.med_patient_ref IN (SELECT pat_id FROM cds2db_in.patient_raw) -- Für alle aktuellen Patienten
--        AND q.hash_index_col NOT IN (SELECT hash_index_col FROM cds2db_in.medication_raw) -- wenn nocht nicht vorhanden durch laden vom FHIR Server
...
--  !!! Aufzählung der FHIR Spalten fehlt noch !!!
--        'reimported from database'
--        FROM cds2db_out.v_location_raw_last_version q    -- View mit letzten FHIR Daten 
--        WHERE q.loc_patient_ref IN (SELECT pat_id FROM cds2db_in.patient_raw) -- Für alle aktuellen Patienten
--        AND q.hash_index_col NOT IN (SELECT hash_index_col FROM cds2db_in.location_raw) -- wenn nocht nicht vorhanden durch laden vom FHIR Server
