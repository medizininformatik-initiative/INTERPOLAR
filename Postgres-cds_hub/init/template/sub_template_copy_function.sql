...
DECLARE
    ...
    data_count integer;
BEGIN
-- Copy Functionname: <%COPY_FKT_NAME%> - From: COPY_FUNC_FROM_SCHEMA -> To: <%OWNER_SCHEMA%>
    FOR current_record IN (SELECT * FROM <%COPY_FUNC_FROM_SCHEMA%>.<%TABLE_NAME%><%CFF_POSTFIX%> WHERE current_dataset_status NOT LIKE 'DELETE after%')
        LOOP
            ...
            FROM <%OWNER_SCHEMA%>.<%TABLE_NAME%><%POSTFIX%> target_record
            ...
            IF data_count=0
            THEN
                INSERT INTO <%OWNER_SCHEMA%>.<%TABLE_NAME%><%POSTFIX%> (
                    pat_id,
	    ...
	    ELSE
	        UPDATE <%OWNER_SCHEMA%>.<%TABLE_NAME%><%POSTFIX%>
...
-- CopyJob Cron-Job - <%COPY_FKT_NAME%> From: COPY_FUNC_FROM_SCHEMA -> To: <%OWNER_SCHEMA%>
