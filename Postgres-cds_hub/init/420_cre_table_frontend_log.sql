...
-- Create time: 2025-06-01 22:54:30
...
------------------------- Index for db_log - patient_fe ---------------------------------
    ...
    IF EXISTS ( -- target column
		...
		DROP INDEX CONCURRENTLY IF EXISTS db_log.idx_patient_fe_id;
   	        CREATE INDEX CONCURRENTLY idx_patient_fe_id ON db_log.patient_fe USING btree (idx_patient_fe_id DESC);
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_patient_fe_input_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_patient_fe_input_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_patient_fe_input_dt ON db_log.patient_fe USING brin (input_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_patient_fe_input_dt ON db_log.patient_fe USING brin (input_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_patient_fe_input_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_patient_fe_input_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_patient_fe_input_pnr ON db_log.patient_fe USING brin (input_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_patient_fe_input_pnr ON db_log.patient_fe USING brin (input_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_patient_fe_last_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_patient_fe_last_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_patient_fe_last_dt ON db_log.patient_fe USING brin (last_check_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_patient_fe_last_dt ON db_log.patient_fe USING brin (last_check_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_patient_fe_last_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_patient_fe_last_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_patient_fe_last_pnr ON db_log.patient_fe USING brin (last_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_patient_fe_last_pnr ON db_log.patient_fe USING brin (last_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_patient_fe_hash',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_patient_fe_hash;
	        CREATE INDEX CONCURRENTLY idx_db_log_patient_fe_hash ON db_log.patient_fe USING btree (hash_index_col);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_patient_fe_hash ON db_log.patient_fe USING btree (hash_index_col);
    END IF; -- INDEX available"%>
...
------------------------- Index for db_log - fall_fe ---------------------------------
    ...
    IF EXISTS ( -- target column
		...
		DROP INDEX CONCURRENTLY IF EXISTS db_log.idx_fall_fe_id;
   	        CREATE INDEX CONCURRENTLY idx_fall_fe_id ON db_log.fall_fe USING btree (idx_fall_fe_id DESC);
            END IF; -- check current status
	ELSE -- (easy) Create new
	    CREATE INDEX CONCURRENTLY idx_fall_fe_id ON db_log.fall_fe USING btree (idx_fall_fe_id DESC);
        END IF; -- INDEX available
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_fall_fe_input_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_fall_fe_input_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_fall_fe_input_dt ON db_log.fall_fe USING brin (input_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_fall_fe_input_dt ON db_log.fall_fe USING brin (input_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_fall_fe_input_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_fall_fe_input_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_fall_fe_input_pnr ON db_log.fall_fe USING brin (input_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_fall_fe_input_pnr ON db_log.fall_fe USING brin (input_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_fall_fe_last_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_fall_fe_last_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_fall_fe_last_dt ON db_log.fall_fe USING brin (last_check_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_fall_fe_last_dt ON db_log.fall_fe USING brin (last_check_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_fall_fe_last_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_fall_fe_last_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_fall_fe_last_pnr ON db_log.fall_fe USING brin (last_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_fall_fe_last_pnr ON db_log.fall_fe USING brin (last_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_fall_fe_hash',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_fall_fe_hash;
	        CREATE INDEX CONCURRENTLY idx_db_log_fall_fe_hash ON db_log.fall_fe USING btree (hash_index_col);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_fall_fe_hash ON db_log.fall_fe USING btree (hash_index_col);
    END IF; -- INDEX available"%>
...
------------------------- Index for db_log - medikationsanalyse_fe ---------------------------------
    ...
    IF EXISTS ( -- target column
		...
		DROP INDEX CONCURRENTLY IF EXISTS db_log.idx_medikationsanalyse_fe_id;
   	        CREATE INDEX CONCURRENTLY idx_medikationsanalyse_fe_id ON db_log.medikationsanalyse_fe USING btree (idx_medikationsanalyse_fe_id DESC);
            END IF; -- check current status
	ELSE -- (easy) Create new
	    CREATE INDEX CONCURRENTLY idx_medikationsanalyse_fe_id ON db_log.medikationsanalyse_fe USING btree (idx_medikationsanalyse_fe_id DESC);
        END IF; -- INDEX available
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_medikationsanalyse_fe_input_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_medikationsanalyse_fe_input_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_medikationsanalyse_fe_input_dt ON db_log.medikationsanalyse_fe USING brin (input_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_medikationsanalyse_fe_input_dt ON db_log.medikationsanalyse_fe USING brin (input_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_medikationsanalyse_fe_input_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_medikationsanalyse_fe_input_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_medikationsanalyse_fe_input_pnr ON db_log.medikationsanalyse_fe USING brin (input_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_medikationsanalyse_fe_input_pnr ON db_log.medikationsanalyse_fe USING brin (input_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_medikationsanalyse_fe_last_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_medikationsanalyse_fe_last_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_medikationsanalyse_fe_last_dt ON db_log.medikationsanalyse_fe USING brin (last_check_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_medikationsanalyse_fe_last_dt ON db_log.medikationsanalyse_fe USING brin (last_check_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_medikationsanalyse_fe_last_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_medikationsanalyse_fe_last_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_medikationsanalyse_fe_last_pnr ON db_log.medikationsanalyse_fe USING brin (last_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_medikationsanalyse_fe_last_pnr ON db_log.medikationsanalyse_fe USING brin (last_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_medikationsanalyse_fe_hash',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_medikationsanalyse_fe_hash;
	        CREATE INDEX CONCURRENTLY idx_db_log_medikationsanalyse_fe_hash ON db_log.medikationsanalyse_fe USING btree (hash_index_col);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_medikationsanalyse_fe_hash ON db_log.medikationsanalyse_fe USING btree (hash_index_col);
    END IF; -- INDEX available"%>
...
------------------------- Index for db_log - mrpdokumentation_validierung_fe ---------------------------------
    ...
    IF EXISTS ( -- target column
		...
		DROP INDEX CONCURRENTLY IF EXISTS db_log.idx_mrpdokumentation_validierung_fe_id;
   	        CREATE INDEX CONCURRENTLY idx_mrpdokumentation_validierung_fe_id ON db_log.mrpdokumentation_validierung_fe USING btree (idx_mrpdokumentation_validierung_fe_id DESC);
            END IF; -- check current status
	ELSE -- (easy) Create new
	    CREATE INDEX CONCURRENTLY idx_mrpdokumentation_validierung_fe_id ON db_log.mrpdokumentation_validierung_fe USING btree (idx_mrpdokumentation_validierung_fe_id DESC);
        END IF; -- INDEX available
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_mrpdokumentation_validierung_fe_input_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_mrpdokumentation_validierung_fe_input_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_mrpdokumentation_validierung_fe_input_dt ON db_log.mrpdokumentation_validierung_fe USING brin (input_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_mrpdokumentation_validierung_fe_input_dt ON db_log.mrpdokumentation_validierung_fe USING brin (input_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_mrpdokumentation_validierung_fe_input_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_mrpdokumentation_validierung_fe_input_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_mrpdokumentation_validierung_fe_input_pnr ON db_log.mrpdokumentation_validierung_fe USING brin (input_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_mrpdokumentation_validierung_fe_input_pnr ON db_log.mrpdokumentation_validierung_fe USING brin (input_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_mrpdokumentation_validierung_fe_last_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_mrpdokumentation_validierung_fe_last_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_mrpdokumentation_validierung_fe_last_dt ON db_log.mrpdokumentation_validierung_fe USING brin (last_check_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_mrpdokumentation_validierung_fe_last_dt ON db_log.mrpdokumentation_validierung_fe USING brin (last_check_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_mrpdokumentation_validierung_fe_last_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_mrpdokumentation_validierung_fe_last_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_mrpdokumentation_validierung_fe_last_pnr ON db_log.mrpdokumentation_validierung_fe USING brin (last_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_mrpdokumentation_validierung_fe_last_pnr ON db_log.mrpdokumentation_validierung_fe USING brin (last_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_mrpdokumentation_validierung_fe_hash',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_mrpdokumentation_validierung_fe_hash;
	        CREATE INDEX CONCURRENTLY idx_db_log_mrpdokumentation_validierung_fe_hash ON db_log.mrpdokumentation_validierung_fe USING btree (hash_index_col);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_mrpdokumentation_validierung_fe_hash ON db_log.mrpdokumentation_validierung_fe USING btree (hash_index_col);
    END IF; -- INDEX available"%>
...
------------------------- Index for db_log - retrolektive_mrpbewertung_fe ---------------------------------
    ...
    IF EXISTS ( -- target column
		...
		DROP INDEX CONCURRENTLY IF EXISTS db_log.idx_retrolektive_mrpbewertung_fe_id;
   	        CREATE INDEX CONCURRENTLY idx_retrolektive_mrpbewertung_fe_id ON db_log.retrolektive_mrpbewertung_fe USING btree (idx_retrolektive_mrpbewertung_fe_id DESC);
            END IF; -- check current status
	ELSE -- (easy) Create new
	    CREATE INDEX CONCURRENTLY idx_retrolektive_mrpbewertung_fe_id ON db_log.retrolektive_mrpbewertung_fe USING btree (idx_retrolektive_mrpbewertung_fe_id DESC);
        END IF; -- INDEX available
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_retrolektive_mrpbewertung_fe_input_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_retrolektive_mrpbewertung_fe_input_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_retrolektive_mrpbewertung_fe_input_dt ON db_log.retrolektive_mrpbewertung_fe USING brin (input_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_retrolektive_mrpbewertung_fe_input_dt ON db_log.retrolektive_mrpbewertung_fe USING brin (input_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_retrolektive_mrpbewertung_fe_input_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_retrolektive_mrpbewertung_fe_input_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_retrolektive_mrpbewertung_fe_input_pnr ON db_log.retrolektive_mrpbewertung_fe USING brin (input_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_retrolektive_mrpbewertung_fe_input_pnr ON db_log.retrolektive_mrpbewertung_fe USING brin (input_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_retrolektive_mrpbewertung_fe_last_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_retrolektive_mrpbewertung_fe_last_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_retrolektive_mrpbewertung_fe_last_dt ON db_log.retrolektive_mrpbewertung_fe USING brin (last_check_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_retrolektive_mrpbewertung_fe_last_dt ON db_log.retrolektive_mrpbewertung_fe USING brin (last_check_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_retrolektive_mrpbewertung_fe_last_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_retrolektive_mrpbewertung_fe_last_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_retrolektive_mrpbewertung_fe_last_pnr ON db_log.retrolektive_mrpbewertung_fe USING brin (last_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_retrolektive_mrpbewertung_fe_last_pnr ON db_log.retrolektive_mrpbewertung_fe USING brin (last_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_retrolektive_mrpbewertung_fe_hash',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_retrolektive_mrpbewertung_fe_hash;
	        CREATE INDEX CONCURRENTLY idx_db_log_retrolektive_mrpbewertung_fe_hash ON db_log.retrolektive_mrpbewertung_fe USING btree (hash_index_col);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_retrolektive_mrpbewertung_fe_hash ON db_log.retrolektive_mrpbewertung_fe USING btree (hash_index_col);
    END IF; -- INDEX available"%>
...
------------------------- Index for db_log - risikofaktor_fe ---------------------------------
    ...
    IF EXISTS ( -- target column
		...
		DROP INDEX CONCURRENTLY IF EXISTS db_log.idx_risikofaktor_fe_id;
   	        CREATE INDEX CONCURRENTLY idx_risikofaktor_fe_id ON db_log.risikofaktor_fe USING btree (idx_risikofaktor_fe_id DESC);
            END IF; -- check current status
	ELSE -- (easy) Create new
	    CREATE INDEX CONCURRENTLY idx_risikofaktor_fe_id ON db_log.risikofaktor_fe USING btree (idx_risikofaktor_fe_id DESC);
        END IF; -- INDEX available
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_risikofaktor_fe_input_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_risikofaktor_fe_input_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_risikofaktor_fe_input_dt ON db_log.risikofaktor_fe USING brin (input_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_risikofaktor_fe_input_dt ON db_log.risikofaktor_fe USING brin (input_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_risikofaktor_fe_input_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_risikofaktor_fe_input_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_risikofaktor_fe_input_pnr ON db_log.risikofaktor_fe USING brin (input_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_risikofaktor_fe_input_pnr ON db_log.risikofaktor_fe USING brin (input_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_risikofaktor_fe_last_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_risikofaktor_fe_last_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_risikofaktor_fe_last_dt ON db_log.risikofaktor_fe USING brin (last_check_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_risikofaktor_fe_last_dt ON db_log.risikofaktor_fe USING brin (last_check_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_risikofaktor_fe_last_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_risikofaktor_fe_last_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_risikofaktor_fe_last_pnr ON db_log.risikofaktor_fe USING brin (last_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_risikofaktor_fe_last_pnr ON db_log.risikofaktor_fe USING brin (last_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_risikofaktor_fe_hash',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_risikofaktor_fe_hash;
	        CREATE INDEX CONCURRENTLY idx_db_log_risikofaktor_fe_hash ON db_log.risikofaktor_fe USING btree (hash_index_col);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_risikofaktor_fe_hash ON db_log.risikofaktor_fe USING btree (hash_index_col);
    END IF; -- INDEX available"%>
...
------------------------- Index for db_log - trigger_fe ---------------------------------
    ...
    IF EXISTS ( -- target column
		...
		DROP INDEX CONCURRENTLY IF EXISTS db_log.idx_trigger_fe_id;
   	        CREATE INDEX CONCURRENTLY idx_trigger_fe_id ON db_log.trigger_fe USING btree (idx_trigger_fe_id DESC);
            END IF; -- check current status
	ELSE -- (easy) Create new
	    CREATE INDEX CONCURRENTLY idx_trigger_fe_id ON db_log.trigger_fe USING btree (idx_trigger_fe_id DESC);
        END IF; -- INDEX available
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_trigger_fe_input_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_trigger_fe_input_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_trigger_fe_input_dt ON db_log.trigger_fe USING brin (input_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_trigger_fe_input_dt ON db_log.trigger_fe USING brin (input_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_trigger_fe_input_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_trigger_fe_input_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_trigger_fe_input_pnr ON db_log.trigger_fe USING brin (input_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_trigger_fe_input_pnr ON db_log.trigger_fe USING brin (input_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_trigger_fe_last_dt',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_trigger_fe_last_dt;
	        CREATE INDEX CONCURRENTLY idx_db_log_trigger_fe_last_dt ON db_log.trigger_fe USING brin (last_check_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_trigger_fe_last_dt ON db_log.trigger_fe USING brin (last_check_datetime);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_trigger_fe_last_pnr',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_trigger_fe_last_pnr;
	        CREATE INDEX CONCURRENTLY idx_db_log_trigger_fe_last_pnr ON db_log.trigger_fe USING brin (last_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_trigger_fe_last_pnr ON db_log.trigger_fe USING brin (last_processing_nr);
    END IF; -- INDEX available"%>
...
IF EXISTS ( -- target column
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db_log_trigger_fe_hash',1,63)
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
		...
		DROP INDEX CONCURRENTLY IF EXISTS idx_db_log_trigger_fe_hash;
	        CREATE INDEX CONCURRENTLY idx_db_log_trigger_fe_hash ON db_log.trigger_fe USING btree (hash_index_col);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX CONCURRENTLY idx_db_log_trigger_fe_hash ON db_log.trigger_fe USING btree (hash_index_col);
    END IF; -- INDEX available"%>
