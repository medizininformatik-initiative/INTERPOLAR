...
CREATE OR REPLACE VIEW cds2db_out.v_encounter AS (
   SELECT DISTINCT
      min(encounter_id) encounter_id, -- use first insert dataset
      enc_id,
      enc_patient_id,
      enc_partof_id,
      enc_identifier_use,
...
CREATE OR REPLACE VIEW cds2db_out.v_patient AS (
   SELECT DISTINCT
      min(patient_id) patient_id, -- use first insert dataset
      pat_id,
      pat_identifier_use,
...
CREATE OR REPLACE VIEW cds2db_out.v_condition AS (
   SELECT DISTINCT
      min(condition_id) condition_id, -- use first insert dataset
      con_id,
      con_encounter_id,
      con_patient_id,
      con_identifier_use,
...
CREATE OR REPLACE VIEW cds2db_out.v_medication AS (
   SELECT DISTINCT
      min(medication_id) medication_id, -- use first insert dataset
      med_id,
      med_identifier_use,
...
CREATE OR REPLACE VIEW cds2db_out.v_medicationrequest AS (
   SELECT DISTINCT
      min(medicationrequest_id) medicationrequest_id, -- use first insert dataset
      medreq_id,
      medreq_encounter_id,
      medreq_patient_id,
...
CREATE OR REPLACE VIEW cds2db_out.v_medicationadministration AS (
   SELECT DISTINCT
      min(medicationadministration_id) medicationadministration_id, -- use first insert dataset
      medadm_id,
      medadm_encounter_id,
      medadm_patient_id,
      medadm_partof_id,
...
CREATE OR REPLACE VIEW cds2db_out.v_medicationstatement AS (
   SELECT DISTINCT
      min(medicationstatement_id) medicationstatement_id, -- use first insert dataset
      medstat_id,
...
CREATE OR REPLACE VIEW cds2db_out.v_observation AS (
   SELECT DISTINCT
      min(observation_id) observation_id, -- use first insert dataset
      obs_id,
      obs_encounter_id,
      obs_patient_id,
      obs_partof_id,
...
CREATE OR REPLACE VIEW cds2db_out.v_diagnosticreport AS (
   SELECT DISTINCT
      min(diagnosticreport_id) diagnosticreport_id, -- use first insert dataset
      diagrep_id,
...
CREATE OR REPLACE VIEW cds2db_out.v_servicerequest AS (
   SELECT DISTINCT
      min(servicerequest_id) servicerequest_id, -- use first insert dataset
      servreq_id,
...
CREATE OR REPLACE VIEW cds2db_out.v_procedure AS (
   SELECT DISTINCT
      min(procedure_id) procedure_id, -- use first insert dataset
      proc_id,
      proc_encounter_id,
      proc_patient_id,
      proc_partof_id,
...
CREATE OR REPLACE VIEW cds2db_out.v_consent AS (
   SELECT DISTINCT
      min(consent_id) consent_id, -- use first insert dataset
      cons_id,
      cons_patient_id,
      cons_identifier_use,
...
CREATE OR REPLACE VIEW cds2db_out.v_location AS (
   SELECT DISTINCT
      min(location_id) location_id, -- use first insert dataset
      loc_id,
      loc_identifier_use,
...
CREATE OR REPLACE VIEW cds2db_out.v_pids_per_ward AS (
   SELECT DISTINCT
      min(pids_per_ward_id) pids_per_ward_id, -- use first insert dataset
      ward_name,
      patient_id
