...
------------------------------------------------------------------------------------------------------------------
-- view in schema db2dataprocessor_out to have access to db-parameters - table is fix
DO
$innerview$
BEGIN
    IF EXISTS ( -- do migration
        ...
        IF EXISTS ( -- VIEW exists
            ...
            WHERE table_schema = 'db2dataprocessor_out' AND table_name = 'v_db_parameter'
        ) THEN
            DROP VIEW db2dataprocessor_out.v_db_parameter; -- first drop the view
        END IF; -- DROP VIEW
----------------------------
        CREATE OR REPLACE VIEW db2dataprocessor_out.v_db_parameter AS (SELECT * FROM db_config.db_parameter ORDER BY parameter_name);

        GRANT SELECT ON db2dataprocessor_out.v_db_parameter TO db2dataprocessor_user;
