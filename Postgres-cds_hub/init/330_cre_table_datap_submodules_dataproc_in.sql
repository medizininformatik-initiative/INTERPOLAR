...
-- Data-leading columns -------------------------------------------------------------------------
        ...
        IF NOT EXISTS ( -- column not exists (enc_mrp_status)
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_calculations' AND column_name = 'enc_mrp_status'
        ) THEN
            ALTER TABLE db2dataprocessor_in.dp_mrp_calculations ADD enc_mrp_status varchar;   -- Fixed values: „1. First Med. Ana. on admission“, „2. First Med. Ana. after operation“, „3. Short stay < 3 days“, „…“ (varchar)
        END IF; -- column (enc_mrp_status)

        IF NOT EXISTS ( -- column not exists (enc_his_identifier)
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_calculations' AND column_name = 'enc_his_identifier'
        ) THEN
            ALTER TABLE db2dataprocessor_in.dp_mrp_calculations ADD enc_his_identifier varchar;   -- optional – Identifier for the medical case in the hospital (Identifier in FHIR) (varchar)
        END IF; -- column (enc_his_identifier)

        IF NOT EXISTS ( -- column not exists (sub_enc_id)
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_calculations' AND column_name = 'sub_enc_id'
        ) THEN
            ALTER TABLE db2dataprocessor_in.dp_mrp_calculations ADD sub_enc_id varchar;   -- optional – FHIR ID Versorgungsstellenkontakt (varchar)
        ...
        IF NOT EXISTS ( -- column not exists (meda_id)
            ...
            ALTER TABLE db2dataprocessor_in.dp_mrp_calculations ADD meda_id varchar;   -- optional – Redcap ID of the medication_analysis_fe (varchar)
        END IF; -- column (meda_id)

        IF NOT EXISTS ( -- column not exists (mrp_type)
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_calculations' AND column_name = 'mrp_type'
        ) THEN
            ALTER TABLE db2dataprocessor_in.dp_mrp_calculations ADD mrp_type varchar;   -- optional – Type of MRP (name of the submodule which has calculated the MRP e.g. “Drug_Disease”, “Drug_Drug”, “Drug_DrugGoup”, “Drug_Kidney”) (varchar)
...
-------------------------------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS db2dataprocessor_in.dp_mrp_ward_type (
  dp_mrp_ward_type_id int PRIMARY KEY DEFAULT nextval('db.db_seq') -- Primary key of the entity
);
...
BEGIN
    IF EXISTS ( -- Table exists
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type'
    ) THEN
        IF NOT EXISTS ( -- column not exists
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'input_datetime'
        ) THEN
            NULL;
        END IF; -- column
...
-- Organizational items - fixed for each database table -----------------------------------------
        IF NOT EXISTS ( -- column not exists
            ...
            ALTER TABLE db2dataprocessor_in.dp_mrp_ward_type ADD input_datetime TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP; -- Time at which data record was created
        END IF; -- column

        IF NOT EXISTS ( -- column not exists
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'last_check_datetime'
        ) THEN
            ALTER TABLE db2dataprocessor_in.dp_mrp_ward_type ADD last_check_datetime TIMESTAMP DEFAULT NULL; -- Time at which data record was last checked
        END IF; -- column

        IF NOT EXISTS ( -- column not exists
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'current_dataset_status'
        ) THEN
            ALTER TABLE db2dataprocessor_in.dp_mrp_ward_type ADD current_dataset_status VARCHAR DEFAULT 'input'; -- Processing status of the data record
        END IF; -- column

        IF NOT EXISTS ( -- column not exists
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'input_processing_nr'
        ) THEN
            ALTER TABLE db2dataprocessor_in.dp_mrp_ward_type ADD input_processing_nr INT; -- (First) Processing number of the data record
        END IF; -- column

        IF NOT EXISTS ( -- column not exists
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'last_processing_nr'
        ) THEN
            ALTER TABLE db2dataprocessor_in.dp_mrp_ward_type ADD last_processing_nr INT; -- Last processing number of the data record
        END IF; -- column

-- Data-leading columns -------------------------------------------------------------------------
        IF NOT EXISTS ( -- column not exists (ward_name)
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'ward_name'
        ) THEN
            ALTER TABLE db2dataprocessor_in.dp_mrp_ward_type ADD ward_name varchar;   -- Unique name of the ward (varchar)
        END IF; -- column (ward_name)

        IF NOT EXISTS ( -- column not exists (ward_type)
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'ward_type'
        ) THEN
            ALTER TABLE db2dataprocessor_in.dp_mrp_ward_type ADD ward_type varchar;   -- Type of the ward, valid values are „surgical“, „internal“ (varchar)
        END IF; -- column (ward_type)


-- Hash column for comparison on data-bearing columns -------------------------------------------
        IF EXISTS ( -- column exists
            SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'hash_index_col'
        ) THEN
            IF NOT EXISTS ( -- column exists
                SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'hash_index_col'
                AND trim(replace(replace(generation_expression::TEXT,'(',''),')','')) != trim(replace(replace('
          ...
          COALESCE(db.to_char_immutable(ward_type), ''#NULL#'') || ''|||'' ||''#''
            ...
            -- Delete the old hash column so that a new one can be created
            ALTER TABLE db2dataprocessor_in.dp_mrp_ward_type DROP COLUMN hash_index_col;

            -- Creating the hash column
            ALTER TABLE db2dataprocessor_in.dp_mrp_ward_type ADD
            hash_index_col TEXT GENERATED ALWAYS AS (
            md5(
...
-- Table "dp_mrp_ward_type" in schema "db2dataprocessor_in"
----------------------------------------------------
GRANT TRIGGER ON db2dataprocessor_in.dp_mrp_ward_type TO db2dataprocessor_user;
GRANT USAGE ON SCHEMA db2dataprocessor_in TO db2dataprocessor_user;
GRANT USAGE ON db.db_seq TO db2dataprocessor_user;
...
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_calculations.enc_mrp_status IS 'Fixed values: „1. First Med. Ana. on admission“, „2. First Med. Ana. after operation“, „3. Short stay < 3 days“, „…“ (varchar)';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_calculations.enc_his_identifier IS 'optional – Identifier for the medical case in the hospital (Identifier in FHIR) (varchar)';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_calculations.sub_enc_id IS 'optional – FHIR ID Versorgungsstellenkontakt (varchar)';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_calculations.meda_id IS 'optional – Redcap ID of the medication_analysis_fe (varchar)';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_calculations.mrp_type IS 'optional – Type of MRP (name of the submodule which has calculated the MRP e.g. “Drug_Disease”, “Drug_Drug”, “Drug_DrugGoup”, “Drug_Kidney”) (varchar)';
...
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_ward_type.dp_mrp_ward_type_id IS 'Primary key of the entity';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_ward_type.ward_name IS 'Unique name of the ward (varchar)';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_ward_type.ward_type IS 'Type of the ward, valid values are „surgical“, „internal“ (varchar)';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_ward_type.input_datetime IS 'Time at which the data record is inserted';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_ward_type.last_check_datetime IS 'Time at which data record was last checked';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_ward_type.current_dataset_status IS 'Processing status of the data record';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_ward_type.input_processing_nr IS '(First) Processing number of the data record';
COMMENT ON COLUMN db2dataprocessor_in.dp_mrp_ward_type.last_processing_nr IS 'Last processing number of the data record';
...
------------------------- Index for db2dataprocessor_in - dp_mrp_ward_type ---------------------------------

-- Index idx_db2dataprocessor_in_dp_mrp_ward_type_input_dt for Table "dp_mrp_ward_type" in schema "db2dataprocessor_in"
----------------------------------------------------
-- Time at which the data record is inserted
IF EXISTS ( -- target column
    SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'input_datetime'
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63) = substr('idx_db2dataprocessor_in_dp_mrp_ward_type_input_dt',1,63) AND schemaname = 'db2dataprocessor_in' AND tablename = 'dp_mrp_ward_type'
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
            AND schemaname = 'db2dataprocessor_in' AND tablename = 'dp_mrp_ward_type' AND substr(indexname,1,63)=substr('idx_db2dataprocessor_in_dp_mrp_ward_type_input_dt',1,63)
	    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_input_dt ON db2dataprocessor_in.dp_mrp_ward_type USING brin (input_datetime)'
        ) THEN -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
            ALTER INDEX db2dataprocessor_in.idx_db2dataprocessor_in_dp_mrp_ward_type_input_dt RENAME TO del_db2dataprocessor_in_dp_mrp_ward_type_i_dt;
	    DROP INDEX IF EXISTS db2dataprocessor_in.del_db2dataprocessor_in_dp_mrp_ward_type_i_dt;
	    CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_input_dt ON db2dataprocessor_in.dp_mrp_ward_type USING brin (input_datetime);
        END IF; -- check current status
    ELSE -- (easy) Create new
        CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_input_dt ON db2dataprocessor_in.dp_mrp_ward_type USING brin (input_datetime);
    END IF; -- INDEX available
...
-- Index idx_db2dataprocessor_in_dp_mrp_ward_type_input_pnr for Table "dp_mrp_ward_type" in schema "db2dataprocessor_in"
----------------------------------------------------
-- (First) Processing number of the data record
IF EXISTS ( -- target column
    SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'input_processing_nr'
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db2dataprocessor_in_dp_mrp_ward_type_input_pnr',1,63) AND schemaname = 'db2dataprocessor_in' AND tablename = 'dp_mrp_ward_type'
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
            AND schemaname = 'db2dataprocessor_in' AND tablename = 'dp_mrp_ward_type' AND substr(indexname,1,63)=substr('idx_db2dataprocessor_in_dp_mrp_ward_type_input_pnr',1,63)
	    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_input_pnr ON db2dataprocessor_in.dp_mrp_ward_type USING brin (input_processing_nr)'
        ) THEN -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
            ALTER INDEX db2dataprocessor_in.idx_db2dataprocessor_in_dp_mrp_ward_type_input_pnr RENAME TO del_db2dataprocessor_in_dp_mrp_ward_type_i_pnr;
	    DROP INDEX IF EXISTS db2dataprocessor_in.del_db2dataprocessor_in_dp_mrp_ward_type_i_pnr;
	    CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_input_pnr ON db2dataprocessor_in.dp_mrp_ward_type USING brin (input_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_input_pnr ON db2dataprocessor_in.dp_mrp_ward_type USING brin (input_processing_nr);
    END IF; -- INDEX available"%>
...
-- Index idx_db2dataprocessor_in_dp_mrp_ward_type_last_dt for Table "dp_mrp_ward_type" in schema "db2dataprocessor_in"
----------------------------------------------------
-- Time at which data record was last checked
IF EXISTS ( -- target column
    SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'last_check_datetime'
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db2dataprocessor_in_dp_mrp_ward_type_last_dt',1,63)  AND schemaname = 'db2dataprocessor_in' AND tablename = 'dp_mrp_ward_type'
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
            AND schemaname = 'db2dataprocessor_in' AND tablename = 'dp_mrp_ward_type' AND substr(indexname,1,63)=substr('idx_db2dataprocessor_in_dp_mrp_ward_type_last_dt',1,63)
	    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_last_dt ON db2dataprocessor_in.dp_mrp_ward_type USING brin (last_check_datetime)'
        ) THEN -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
            ALTER INDEX db2dataprocessor_in.idx_db2dataprocessor_in_dp_mrp_ward_type_last_dt RENAME TO del_db2dataprocessor_in_dp_mrp_ward_type_l_dt;
	    DROP INDEX IF EXISTS db2dataprocessor_in.del_db2dataprocessor_in_dp_mrp_ward_type_l_dt;
	    CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_last_dt ON db2dataprocessor_in.dp_mrp_ward_type USING brin (last_check_datetime);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_last_dt ON db2dataprocessor_in.dp_mrp_ward_type USING brin (last_check_datetime);
    END IF; -- INDEX available"%>
...
-- Index idx_db2dataprocessor_in_dp_mrp_ward_type_last_pnr for Table "dp_mrp_ward_type" in schema "db2dataprocessor_in"
----------------------------------------------------
-- Last processing number of the data record
IF EXISTS ( -- target column
    SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'last_processing_nr'
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db2dataprocessor_in_dp_mrp_ward_type_last_pnr',1,63) AND schemaname = 'db2dataprocessor_in' AND tablename = 'dp_mrp_ward_type'
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
            AND schemaname = 'db2dataprocessor_in' AND tablename = 'dp_mrp_ward_type' AND substr(indexname,1,63)=substr('idx_db2dataprocessor_in_dp_mrp_ward_type_last_pnr',1,63)
	    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_last_pnr ON db2dataprocessor_in.dp_mrp_ward_type USING brin (last_processing_nr)'
        ) THEN -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
            ALTER INDEX db2dataprocessor_in.idx_db2dataprocessor_in_dp_mrp_ward_type_last_pnr RENAME TO del_db2dataprocessor_in_dp_mrp_ward_type_l_pnr;
            DROP INDEX IF EXISTS db2dataprocessor_in.del_db2dataprocessor_in_dp_mrp_ward_type_l_pnr;
	    CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_last_pnr ON db2dataprocessor_in.dp_mrp_ward_type USING brin (last_processing_nr);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_last_pnr ON db2dataprocessor_in.dp_mrp_ward_type USING brin (last_processing_nr);
    END IF; -- INDEX available"%>
...
-- Index idx_db2dataprocessor_in_dp_mrp_ward_type_hash for Table "dp_mrp_ward_type" in schema "db2dataprocessor_in"
----------------------------------------------------
-- Column for automatic hash value for comparing FHIR data
IF EXISTS ( -- target column
    SELECT 1 FROM information_schema.columns WHERE table_schema = 'db2dataprocessor_in' AND table_name = 'dp_mrp_ward_type' AND column_name = 'hash_index_col'
    ...
    IF EXISTS ( -- INDEX available
        SELECT 1 FROM pg_indexes where substr(indexname,1,63)=substr('idx_db2dataprocessor_in_dp_mrp_ward_type_hash',1,63) AND schemaname = 'db2dataprocessor_in' AND tablename = 'dp_mrp_ward_type'
    ) THEN -- check current status
        IF EXISTS ( -- INDEX nicht auf akuellen Stand
            SELECT 1 FROM pg_indexes WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
            AND schemaname = 'db2dataprocessor_in' AND tablename = 'dp_mrp_ward_type' AND substr(indexname,1,63)=substr('idx_db2dataprocessor_in_dp_mrp_ward_type_hash',1,63)
	    AND indexdef != 'CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_input_dt ON db2dataprocessor_in.dp_mrp_ward_type USING btree (hash_index_col)'
        ) THEN -- Index entspricht nicht aktuellen Stand - deshalb Index löschen und neu anlegen
            ALTER INDEX db2dataprocessor_in.idx_db2dataprocessor_in_dp_mrp_ward_type_hash RENAME TO del_db2dataprocessor_in_dp_mrp_ward_type_hash;
	    DROP INDEX IF EXISTS db2dataprocessor_in.del_db2dataprocessor_in_dp_mrp_ward_type_hash;
	    CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_hash ON db2dataprocessor_in.dp_mrp_ward_type USING btree (hash_index_col);
        END IF; -- check current status"%>
    ELSE -- (easy) Create new
        CREATE INDEX idx_db2dataprocessor_in_dp_mrp_ward_type_hash ON db2dataprocessor_in.dp_mrp_ward_type USING btree (hash_index_col);
    END IF; -- INDEX available"%>
