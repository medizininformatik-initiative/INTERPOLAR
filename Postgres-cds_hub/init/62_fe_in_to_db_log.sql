...
-- Create time: 2025-03-03 22:36:39
...
BEGIN
    ...
    UNION SELECT COUNT(1) AS anz FROM db2frontend_in.retrolektive_mrp_bewertung_fe
    ...
    IF data_count_pro_all>0 THEN
        ...
        SELECT COUNT(1) INTO data_count_all FROM db2frontend_in.retrolektive_mrp_bewertung_fe; -- Counting new records in the source
        ...
        IF data_count_all>0 THEN -- Complete execution is only necessary if new data records are available - otherwise no database access is necessary
            ...
            err_section:='retrolektive_mrp_bewertung_fe-05';    err_schema:='db2frontend_in';    err_table:='retrolektive_mrp_bewertung_fe';

            FOR current_record IN (SELECT * FROM db2frontend_in.retrolektive_mrp_bewertung_fe)
                LOOP
                    BEGIN
                        IF last_pro_nr IS NULL THEN SELECT nextval('db.db_seq') INTO last_pro_nr; END IF; -- Get the processing number for this process only if records found
                        ...
                        THEN
                            ...
                            -- Delete importet datasets
                            err_section:='retrolektive_mrp_bewertung_fe-20';    err_schema:='db2frontend_in';    err_table:='retrolektive_mrp_bewertung_fe';
                            DELETE FROM db2frontend_in.retrolektive_mrp_bewertung_fe WHERE retrolektive_mrp_bewertung_fe_id = current_record.retrolektive_mrp_bewertung_fe_id;
                        ELSE
                            ...
                            -- Delete updatet datasets
                            err_section:='retrolektive_mrp_bewertung_fe-30';    err_schema:='db2frontend_in';    err_table:='retrolektive_mrp_bewertung_fe';
                            DELETE FROM db2frontend_in.retrolektive_mrp_bewertung_fe WHERE retrolektive_mrp_bewertung_fe_id = current_record.retrolektive_mrp_bewertung_fe_id;
                        END IF;
                    EXCEPTION
                        WHEN OTHERS THEN
                            err_section:='retrolektive_mrp_bewertung_fe-35';    err_schema:='db2frontend_in';    err_table:='retrolektive_mrp_bewertung_fe';
                            UPDATE db2frontend_in.retrolektive_mrp_bewertung_fe
                            SET last_check_datetime = last_pro_datetime
                            , current_dataset_status = 'ERROR func: copy_fe_fe_in_to_db_log'
                            , last_processing_nr = last_pro_nr
                    ...
                    err_section:='retrolektive_mrp_bewertung_fe-40';    err_schema:='db2frontend_in';    err_table:='retrolektive_mrp_bewertung_fe';
                    IF data_count_last_status_set>=COALESCE(data_count_last_status_max,10) THEN -- Info ausgeben
                        SELECT res FROM pg_background_result(pg_background_launch(
                        'UPDATE db_config.db_process_control set pc_value='''||data_count_pro_processed||''', last_change_timestamp=CURRENT_TIMESTAMP
                        WHERE pc_name=''currently_processed_number_of_data_records_in_the_function'''
            ...
            INSERT INTO db.data_import_hist (last_processing_nr, variable_name, schema_name, table_name, last_check_datetime, function_name, dataset_count, copy_time_in_sec, current_dataset_status)
            VALUES ( last_pro_nr,'data_count_new', 'db_log', 'retrolektive_mrp_bewertung_fe', last_pro_datetime, 'copy_fe_fe_in_to_db_log', data_count_new, tmp_sec, temp);

            INSERT INTO db.data_import_hist (last_processing_nr, variable_name, schema_name, table_name, last_check_datetime, function_name, dataset_count, copy_time_in_sec, current_dataset_status)
            VALUES ( last_pro_nr,'data_count_update', 'db_log', 'retrolektive_mrp_bewertung_fe', last_pro_datetime, 'copy_fe_fe_in_to_db_log', data_count_update, tmp_sec, temp);

            INSERT INTO db.data_import_hist (last_processing_nr, variable_name, schema_name, table_name, last_check_datetime, function_name, dataset_count, copy_time_in_sec, current_dataset_status)
            VALUES ( last_pro_nr,'data_count_all', 'db_log', 'retrolektive_mrp_bewertung_fe', last_pro_datetime, 'copy_fe_fe_in_to_db_log', data_count_all, tmp_sec, temp);
        END IF; -- Complete execution is only necessary if new data records are available - otherwise no database access is necessary
