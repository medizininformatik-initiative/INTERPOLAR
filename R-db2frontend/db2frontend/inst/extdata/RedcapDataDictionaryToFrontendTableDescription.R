...
for (i in 1:nrow(dt)) {
  ...
  # If it is a checkbox, split it into several lines
  if (row$COLUMN_TYPE == "checkbox") {
    options <- trimws(unlist(strsplit(row$CHOICES, "\\|")))  # Split bei "|"

    # Create a new line for each option
    for (option in options) {
      option <- gsub(",", " -", option)  # Replace comma with “ -”
      option_number <- gsub(" -.*", "", option)  # Extract the number before the separator
      new_row <- data.table(
        TABLE_NAME = row$TABLE_NAME,
        COLUMN_NAME = paste0(row$COLUMN_NAME, "___", option_number),
        COLUMN_DESCRIPTION = option,  # Keep the full text
        COLUMN_TYPE = row$COLUMN_TYPE,  # Keep the original type for mapping
...
# Add the "This file is generated..." header to the result file
header <- c(
  "Hint",
