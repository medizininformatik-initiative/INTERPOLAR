...
getMRPTableColumnNames <- function() {
  # these are *exactly* the names of the columns in the excel file
  etlutils::namedListByValue("MEDICATION_NAME",
                             "ATC_DISPLAY",
                             "ATC",
                             "CONDITION_DISPLAY",
                             "CONDITION_DISPLAY_2",
...
#'
#' @details
#' The function checks whether a previously processed version of the table is stored in RDS files.
#' If not, it calls a cleaning and expansion function named `cleanAndExpandDefinition<name>`,
#' where `<name>` is derived from the `table_name` parameter (hyphens removed).
#'
getExpandedContent <- function(table_name, path_to_mrp_tables) {
  # Load the MRP definition from the Excel file
  mrp_columnnames <- getMRPTableColumnNames()
