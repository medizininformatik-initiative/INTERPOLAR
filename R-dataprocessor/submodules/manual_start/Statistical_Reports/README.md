...
- die Tabellen enthalten folgende Kennzahlen:
  ...
  - Anzahl der Fälle, für die mindestens ein MRP algorithmisch ermittelt wurde sowie ein consent vorliegt und das MRP in der retrolektiven Bewertung als "nicht bestätigt" markiert wurde, dies jedoch nicht aufgrund einer falschen Datengrundlage erfolgte (encounters with non-confirmed non-incorrect data items MRP and consent)
...
- falls es für einen Fall verschiedene enc_identifier_values geben kann, muss über MEDICAL_CASE_ID_ENCOUNTER_FHIR_IDENTIFIER_SYSTEM in der dataprocessor_config.toml festgelegt werden, aus welchem System die eigentliche Aufnahmenummer/Fallnummer stammt, dieses muss dann für alle drei Encounter-Ebenen eingetragen sein
- wenn MEDICAL_CASE_ID_ENCOUNTER_FHIR_IDENTIFIER_SYSTEM spezifiziert ist, werden Datenbankreihen mit anderem (oder NA) enc_identifier_system nicht in der Auswertung beachtet
