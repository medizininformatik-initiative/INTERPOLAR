...
#'   \item{`main_enc_any_processing_exclusion`}{Logical flag indicating if any processing exclusion reason exists for the main encounter}
#'
#' @details
#' \itemize{
...
#' - `main_enc_any_processing_exclusion_fe` (indicating if any processing exclusion reason exists for the main encounter)
#'
...
#'
#' @param F1_data A data frame containing F1 base population metrics, typically generated by `prepareF1data()`.
#' @param frontend_summary_data A data frame of front-end documentation data, typically generated by `prepareFeSummaryData()`.
#'
#' @return A data frame where `F1_data` is enriched with selected variables from `frontend_summary_data`,
#' joined on patient and encounter keys. If multiple rows match, the one with the earliest `meda_dat` (medication analysis date)
#' per group is selected using `selectMin()`.
#'
#' @details
...
#' }
#'
#' The function retains only one row per group of interest using `selectMin()` based on the minimum `meda_dat`.
#'
...
addFeDataToF1data <- function(F1_data, frontend_summary_data) {
  ...
  F1_data_with_fe <- F1_data |>
    dplyr::left_join(frontend_summary_data |>
                       dplyr::distinct(pat_id, main_enc_id, enc_id, record_id, fall_id_cis,
                     ...
                     by = c("pat_id","main_enc_id","enc_id", "record_id",
                            "fall_id_cis","ward_name")) |>
    selectMin(grouping_variables = c("pat_id","main_enc_id","enc_id",
