...
# Installationsanleitung bzw. Anleitung zur Einrichtung der CDS tool chain
  ...
  1. Nehmen Sie Anpassungen in der [docker-compose.yml](docker-compose.yml) vor,
  ...
  1. Datenbank einrichten (Neuinstallation) oder anpassen (Migration). Dieser Schritt muss manuell über den folgenden Befehl erfolgen:
     ...
     ```
  1. Die cds_hub_db (Postges-Datenbank) erreichen Sie im Browser (PGAdmin) über
     die URL: [http://127.0.0.1:8089/](http://127.0.0.1:8089/), wenn der
     optionale Compose-Profile-Service `pgadmin` gestartet wurde, z.B. mit
     `docker compose --profile pgadmin up`.
     * Die Zugangsdaten für pgadmin entnehmen Sie bitte der [docker-compose.yml](docker-compose.yml) (services -> pgadmin) bzw. können Sie diese dort anpassen.
     * ggf. muss im pgadmin die Verbindung zur cds_hub_db mit den Zugangsdaten aus der [docker-compose.yml](docker-compose.yml) (services -> cds_hub: POSTGRES_USER, POSTGRES_DB) bzw. der Passwort-Datei (secrets -> cds_hub_db_admin.password, Postgres-cds_hub/.env_cds_hub_db_admin.password) angelegt werden. Weitere Informationen siehe [Postgres-cds_hub/Readme.md](Postgres-cds_hub/Readme.md)
