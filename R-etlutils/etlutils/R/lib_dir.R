# Environment for saving global variables for this file
.lib_dir_env <- new.env()
...
#'
getProjectDirNames <- function(project_name, project_time_stamp) {
...
#'
renameWithCreationTimeIfDirExists <- function(dir, max_dir_count, timeStampPrefix = "") {
  newName <- NA
  if (dir.exists(dir)) {
...
#' @param max_dir_count maximum of old project directories, higher numbers will be deleted
#' @param use_timestamp_as_result_dir_suffix logical; indicating result directories have project
#' time stamp as suffix
#' @param showWarnings logical; should the warnings on failure be shown?
#'
#' @export
createDIRS <- function(project_name, max_dir_count, use_timestamp_as_result_dir_suffix = TRUE, showWarnings = FALSE) {
  .lib_dir_env[["PROJECT_NAME"]] <- project_name
  # The result dir can be extended by an timestamp. To enable timestamp suffixes at the result
  # dir set. The variable USE_TIMESTAMP_AS_RESULT_DIR_SUFFIX = true in the config toml file.
  project_time_stamp <- ifelse(use_timestamp_as_result_dir_suffix, format(Sys.time(), "-%Y-%m%d-%H%M%S"), "")
  .lib_dir_env[["PROJECT_TIME_STAMP"]] <- project_time_stamp

  sub_project_dirs <- getProjectDirNames(project_name, project_time_stamp)
  .lib_dir_env[["SUB_PROJECT_DIRS"]] <- sub_project_dirs

  renameWithCreationTimeIfDirExists(sub_project_dirs$global_dir, max_dir_count)
  renameWithCreationTimeIfDirExists(sub_project_dirs$local_dir, max_dir_count)
  for (rd in sub_project_dirs$global_results_directories_names) {
    dir.create(paste0(sub_project_dirs$global_dir, "/", rd), recursive = TRUE, showWarnings = showWarnings)
  }
  for (rd in sub_project_dirs$local_results_directories_names) {
    dir.create(paste0(sub_project_dirs$local_dir, "/", rd), recursive = TRUE, showWarnings = showWarnings)
  }
}
...
#'
returnPathToLogDir <- function() {
  fhircrackr::pastep(.lib_dir_env[["SUB_PROJECT_DIRS"]]$local_dir, "log")
}
...
returnPathToBundlesDir <- function() {
  fhircrackr::pastep(.lib_dir_env[["SUB_PROJECT_DIRS"]]$local_dir, "bundles")
}
...
savePerformance <- function(filename_without_extension = "Performance_informations", clock = getClock()) {
  clock$write(filename_without_extension = fhircrackr::pastep(.lib_dir_env[["SUB_PROJECT_DIRS"]]$local_dir, "performance", filename_without_extension), hide_errors = FALSE)
  clock$write(filename_without_extension = fhircrackr::pastep(.lib_dir_env[["SUB_PROJECT_DIRS"]]$global_dir, "performance", filename_without_extension), hide_errors = TRUE)
}
...
writeRData <- function(object = table, filename_without_extension = NA, project_sub_dir = NA) {
  ...
  if (is.na(project_sub_dir)) {
    project_sub_dir <- fhircrackr::pastep(.lib_dir_env[["SUB_PROJECT_DIRS"]]$local_dir, "tables")
  } else {
...
getLocalRdataFileName <- function(table_name) {
  fhircrackr::pastep(.lib_dir_env[["SUB_PROJECT_DIRS"]]$local_dir, "tables", table_name, ext = '.RData')
}
